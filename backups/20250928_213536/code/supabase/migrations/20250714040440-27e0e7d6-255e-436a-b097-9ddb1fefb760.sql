...
CREATE POLICY "Authenticated users can manage their own likes" ON public.question_likes
FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Authenticated users can like questions" ON public.question_likes
...
-- Keep one public view policy for course stages (needed for public access)
CREATE POLICY "Public can view course stages" ON public.course_stages
...
-- Keep one public view policy for courses (needed for public access)
...
-- Keep one public view policy for stage questions (needed for public access)
CREATE POLICY "Public can view stage questions" ON public.stage_questions
FOR SELECT USING (true);
