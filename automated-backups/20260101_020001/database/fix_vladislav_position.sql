-- SPECIFIC FIX: Position not loading for Vladislav Kan and others
-- Run this in your Supabase SQL Editor

-- 1. First, let's check if Vladislav has a position assigned
...
-- 3. Set position for Vladislav specifically
UPDATE profiles 
SET position = 'Engineering Manager' 
WHERE email = 'vladislav@source-edge.com';

-- 4. Check if the position was set
SELECT id, email, full_name, role, position 
FROM profiles 
WHERE email = 'vladislav@source-edge.com';

-- 5. CRITICAL: Update the database function to include position field
...
-- 6. Test the function specifically for Vladislav
SELECT * FROM get_users_by_stage_with_rejected('Interested', FALSE) 
WHERE email = 'vladislav@source-edge.com';

-- 7. Test the function for all users in Interested stage
