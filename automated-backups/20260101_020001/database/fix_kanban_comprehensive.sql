-- Comprehensive fix for Kanban board issues
-- This handles the company field properly and ensures users are visible

-- 1. Ensure company column exists
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS company TEXT;

-- 2. Set default company for all existing Kanban users
UPDATE profiles 
SET 
    company = COALESCE(company, 'Woven by Toyota'),
    updated_at = NOW()
WHERE id IN (
...
-- 3. Drop and recreate the function with proper company handling
...
BEGIN
    ...
    SELECT 
        ...
        p.role::TEXT,
        p."position"::TEXT,
        COALESCE(p.company, 'Woven by Toyota')::TEXT as company,
