-- Fix the get_users_by_stage function to show all users by default
...
-- Also create a simpler version that works without the rejected parameter for backward compatibility
CREATE OR REPLACE FUNCTION get_users_by_stage_simple(p_stage VARCHAR(50))
RETURNS TABLE (
  user_id UUID,
  email TEXT,
  full_name TEXT,
