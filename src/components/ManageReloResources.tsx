...
const PAGE_IDENTIFIER = "relo";

export const ManageReloResources = ({ onSuccess }: { onSuccess?: () => void }) => {
  ...
  const { data: currentResources, isLoading: isLoadingCurrent } = useQuery({
    queryKey: ['current-page-resources', PAGE_IDENTIFIER],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('page_resources')
        .select('resource_id')
  ...
  const categories = Array.from(new Set((allResources || []).map(r => r.category).filter(Boolean))).sort();

  const handleSave = async () => {
    setIsSaving(true);
    try {
      // First, remove all existing resources for this page
      ...
      queryClient.invalidateQueries({ queryKey: ['page-resources', PAGE_IDENTIFIER] });
      queryClient.invalidateQueries({ queryKey: ['current-page-resources', PAGE_IDENTIFIER] });

      toast({
        title: "Resources updated!",
        description: `Successfully assigned ${selectedResources.size} resources to the Relo page.`,
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Select Resources for Relo Page ({selectedResources.size} selected)</h3>
      ...
      <div className="flex gap-4 items-center">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 transform -tranneutral-y-1/2 text-neutral-500 h-4 w-4" />
          <Input
        ...
        {filteredResources.length === 0 ? (
          ...
          <div className="overflow-hidden">
            <table className="w-full table-fixed">
              <thead className="bg-neutral-800/50 border-b">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider w-12">
                    Select
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider w-1/4">
                    Title
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider w-20">
                    Category
                  </th>
