...
// Social login is currently disabled in Omiyages.
// Keep the provider list for future use, but do not render any OAuth buttons.
...
export const AuthModalDialog: React.FC<AuthModalDialogProps> = ({ isOpen, onClose, initialMode }) => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signIn.status === 'complete') {
        ...
        setCode('');
        try { await signIn.emailCode.sendCode(); } catch {}
        setMode('mfa');
      } else {
        setError(SIGN_IN_ERROR);
      }
          ...
          {mode === 'mfa' ? (
            <form onSubmit={handleMfa} className="w-full">
              ...
              <p className="text-neutral-500 text-xs text-center mb-5 leading-relaxed">
                ...
                <button
                  type="button"
                  onClick={async () => { try { await signIn?.emailCode.sendCode(); } catch {} }}
            ...
            <>
              ...
              {/* OAuth providers are intentionally hidden for now. */}
            </>
          )}
        </div>
      </DialogContent>
